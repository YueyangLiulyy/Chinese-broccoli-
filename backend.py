...
class backEnd:
    def __init__(self, masterFile, transactionsFile):
        self.transactionsList = self.readFile(transactionsFile)                 # they are all lists
    ...
    def updateValidAccList(self):
        ...
        for master in self.masterList:
            ...
            validAccountsList.append(acctNum)               # add account number to master list
    ...
    def preProcessing(self, transact):
        ...
        name = transaction[4]
        if opreation == "DEP":
            self.processDeposit(toAccount, amount, fromAccount)
        elif opreation == "WDR":
            self.processWithdraw(toAccount, amount, fromAccount)
        elif opreation == "XFR":
            self.processTransfer(toAccount, amount, fromAccount)
        elif opreation == "NEW":
            self.processCreate(toAccount, amount, fromAccount, name)
        elif opreation == "DEL":
            self.processDelete(toAccount, amount, fromAccount, name)
        else:
    ...
    def processDeposit(self, toAccount, amount, fromAccount):
        ...
        if match:
            ...
            name = increaseAccount[2].strip()                                   # may have '\n' after name 
    ...
    def processWithdraw(self, toAccount, amount, fromAccount):
        ...
        if match:
            ...
            name = decreaseAccount[2].strip()                                   # may have '\n' after name
    ...
    def processTransfer(self, toAccount, amount, fromAccount):
        ...
        if match and match2:
            ...
            name = decreaseAccount[2].strip()                                   # may have '\n' after name
            name2 = increaseAccount[2].strip()                                  # may have '\n' after name
            if newAmount < 0 :
...
def main():
    ...
    summaryDir= currentDir + "/summary.txt"
    masterFile= currentDir + "/master.txt"
    backEnd(masterFile, summaryDir)
