...
class AGENT(ATM):
    ...
    def functionDeleteAccount(self):
        # Check account number
        accountNumber = input("Enter the account number you want to delete please: ")
        if accountNumber == "logout":
            return 0
    ...
    def verifyAccountName(self, accountName):
        if len(accountName) < 3 or len(accountName) > 30:
            print("the account name should between 3 and 30 alphanumeric characters: ")
            return False
        elif accountName[0] == ' ' or accountName[-1] == ' ':
            print( "the account name should not beginning or ending with a space: ")
            return False
        else:
            return True
    ...
    def greeting(self):
        print("Hello, welcome to the agent mode, please type the transaction you want to make: ")
    ...
    def verifyDepositAmount(self, account, amount):
        if super().verifyAmount(amount):
            if super().inputConvert(amount) > 999999.99:                                                # each time deposit should <= $999,999.99
    ...
    def verifyWithdrawAmount(self, account, amount):
        if super().verifyAmount(amount):
            if super().inputConvert(amount) > 999999.99:                                                # each time withdraw should <= $999,999.99
    ...
    def verifyTransferAmount(self, fromAccount, amount):
        if super().verifyAmount(amount):
            if super().inputConvert(amount) > 999999.99:                                                # each transfer should <= $999,999.99
